  ...
  const saveSnippetHandler = () => {
    ...
    if (key?.trim() && value?.trim()) {
      chrome.storage.sync.set({ [key]: value }, () =>
        setMessage([true, "variable has been set"])
      );
    } else {
    ...
    <Container>
      <form style={{ width: "100%" }}>
        <ModalInput
          ...
          id="key"
          ref={shortcutRef}
          placeholder="Shortcut for the snippet"
        />
        <TextArea
          id="value"
          ref={snippetTextRef}
      ...
      <ButtonContainer>
        <Button type="button" onClick={saveSnippetHandler}>
          Save
        </Button>
        <Button type="button" onClick={editSnippetHandler}>
          Edit Variables
        </Button>
      </ButtonContainer>
      ...
      <form>
        <LookupWrapper>
          ...
          />
          <LookupButton id="find" type="button" onClick={findSnippetHandler}>
            Find
          </LookupButton>
        </LookupWrapper>
      </form>
      ...
      <ValueFound>
        {lookupValue.value != "" && (
          <>
            ...
            {lookupValue.exist && (
              <Button onClick={copyBtnHandler} disabled={copyBtnAtt.disabled}>
                {copyBtnAtt.innerText}
              </Button>
            )}
          </>
        )}
